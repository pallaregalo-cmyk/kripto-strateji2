...
def open_pos(symbol, side, usdt_amount, sl_pct, tp_pct):
    ...
    if side == "BUY":
        ...
        sl_price = round(price * (1 + sl_pct / 100), pp)
        tp_price = round(price * (1 - tp_pct / 100), pp)
    print(f"Bot SL={sl_price} TP={tp_price}")
    return {"entry_price": price, "sl_price": sl_price, "tp_price": tp_price, "side": side, "qty": qty}


def get_klines_full(symbol, interval, limit=300):
    r = requests.get(
...
def get_klines(symbol, interval, limit=300):
    return [float(k[4]) for k in get_klines_full(symbol, interval, limit)]
...
def calc_macd(prices, fast, slow, signal):
    """MACD hesapla. Son değerleri döndürür: (macd_line, signal_line, histogram)"""
    ...
    macd_prev = macd_line[-2] if len(macd_line) >= 2 else macd_now
    sig_prev_val = sig
    if len(macd_line) >= signal + 2:
        sig2 = sum(macd_line[:signal]) / signal
        for v in macd_line[signal:-1]:
            sig2 = v * k + sig2 * (1 - k)
        sig_prev_val = sig2
    return macd_now, sig, macd_prev, sig_prev_val
...
def bot_loop(user_id, strategy):
    ...
    macd_fast = strategy.get("macd_fast", 12) or 12
    macd_slow = strategy.get("macd_slow", 26) or 26
    macd_signal = strategy.get("macd_signal", 9) or 9
    ema_trend = strategy.get("ema_trend", 200) or 200
    vol_period = strategy.get("vol_period", 20) or 20
    ...
    # Aktif parametreleri status'a yaz
    active_bots[user_id]["active_params"] = {
    ...
    if strategy_type == "sma":
        active_bots[user_id]["active_params"].update({"sma1": sma1_p, "sma2": sma2_p})
    elif strategy_type == "bb":
        active_bots[user_id]["active_params"].update({"bb_period": bb_period, "bb_std": bb_std})
    elif strategy_type == "ema":
        active_bots[user_id]["active_params"].update({"ema1": ema1_p, "ema2": ema2_p, "volume_filter": volume_filter})
    elif strategy_type == "macd":
        active_bots[user_id]["active_params"].update({
            "macd_fast": macd_fast, "macd_slow": macd_slow, "macd_signal": macd_signal,
            "ema_trend": ema_trend, "vol_period": vol_period
        })
    ...
    while active_bots.get(user_id, {}).get("running", False):
        try:
            ...
            if active_position:
                ...
                if sl_hit:
                    print(f"[{datetime.now()}] STOP LOSS! Fiyat={current_price} SL={sl}")
                    position = get_position(symbol)
                    if position != 0:
                    ...
                    initialized = False
                    active_bots[user_id]["active_position"] = None
                    active_bots[user_id]["last_error"] = f"SL @ {current_price}"
                    time.sleep(5)
                    continue

                if tp_hit:
                    print(f"[{datetime.now()}] TAKE PROFIT! Fiyat={current_price} TP={tp}")
                    position = get_position(symbol)
                    if position != 0:
                    ...
                    initialized = False
                    active_bots[user_id]["active_position"] = None
                    active_bots[user_id]["last_error"] = f"TP @ {current_price}"
                    time.sleep(5)
                    continue
                ...
                if strategy_type == "ema" and side == "BUY":
                    ...
                    if ema200_now and current_price < ema200_now:
                        print(f"[{datetime.now()}] EMA SL: fiyat EMA{ema2_p} altinda")
                        position = get_position(symbol)
                        if position != 0:
                        ...
                        initialized = False
                        active_bots[user_id]["active_position"] = None
                        active_bots[user_id]["last_error"] = f"EMA SL @ {current_price}"
                        time.sleep(5)
                        continue

            # Veri çek
            limit = max(sma1_p, sma2_p, bb_period, rsi_period, ema2_p, macd_slow + macd_signal, ema_trend, vol_period) + 30
            ...
            if strategy_type == "sma":
                ...
                active_bots[user_id]["ind1_val"] = round(s1, 4)
                active_bots[user_id]["ind2_val"] = round(s2, 4)
                active_bots[user_id]["ind1_label"] = f"SMA{sma1_p}"
                active_bots[user_id]["ind2_label"] = f"SMA{sma2_p}"
                if not initialized:
                    prev_signal = "BUY" if s1 > s2 else "SELL"
                    initialized = True
                    active_bots[user_id]["status"] = "running"
                    print(f"Bot hazir. SMA{sma1_p}={s1:.4f} SMA{sma2_p}={s2:.4f}. Kesisim bekleniyor...")
                    time.sleep(sleep_sec)
                    continue
                if s1_prev <= s2_prev and s1 > s2:
                    signal = "BUY"
                elif s1_prev >= s2_prev and s1 < s2:
                    signal = "SELL"
                print(f"[{now}] SMA{sma1_p}={s1:.4f} SMA{sma2_p}={s2:.4f} RSI={rsi_val}")
            ...
            # ── BB ──
                ...
                active_bots[user_id]["ind1_val"] = round(upper, 4)
                active_bots[user_id]["ind2_val"] = round(lower, 4)
                active_bots[user_id]["ind1_label"] = f"BB Üst({bb_period})"
                active_bots[user_id]["ind2_label"] = f"BB Alt({bb_period})"
                if not initialized:
                    initialized = True
                    active_bots[user_id]["status"] = "running"
                    print(f"Bot hazir. BB Ust={upper} Alt={lower}.")
                    time.sleep(sleep_sec)
                    continue
            ...
            # ── EMA ──
                ...
                active_bots[user_id]["ind1_val"] = round(e1_now, 4)
                active_bots[user_id]["ind2_val"] = round(e2_now, 4)
                active_bots[user_id]["ind1_label"] = f"EMA{ema1_p}"
                active_bots[user_id]["ind2_label"] = f"EMA{ema2_p}"
                if not initialized:
                    prev_signal = "BUY" if e1_now > e2_now else "SELL"
                    initialized = True
                    active_bots[user_id]["status"] = "running"
                    print(f"Bot hazir. EMA{ema1_p}={e1_now:.4f} EMA{ema2_p}={e2_now:.4f}. Kesisim bekleniyor...")
                    time.sleep(sleep_sec)
                    continue
                golden = e1_prev <= e2_prev and e1_now > e2_now
                death = e1_prev >= e2_prev and e1_now < e2_now
                if golden:
                    vol_ok = True
                    ...
                    if vol_ok and rsi_ok:
                        ...
                        print(f"Golden Cross filtreler gecmedi: Vol={vol_ok} RSI={rsi_val}")
                elif death:
                    rsi_ok_sell = rsi_val is None or rsi_val < 50
                    if rsi_ok_sell:
                        signal = "SELL"
                    else:
                        print(f"Death Cross RSI filtresi gecmedi: RSI={rsi_val}")
                print(f"[{now}] EMA{ema1_p}={e1_now:.4f} EMA{ema2_p}={e2_now:.4f} RSI={rsi_val}")

            # ── MACD + EMA Trend + Hacim ──
            elif strategy_type == "macd":
                macd_result = calc_macd(prices, macd_fast, macd_slow, macd_signal)
                if macd_result[0] is None:
                    time.sleep(10)
                    continue
                macd_now, sig_now, macd_prev_val, sig_prev_val = macd_result

                # EMA trend filtresi
                ema_trend_val = calc_ema(prices, ema_trend)
                current_price = prices[-1]
                trend_up = ema_trend_val is not None and current_price > ema_trend_val
                trend_down = ema_trend_val is not None and current_price < ema_trend_val

                active_bots[user_id]["ind1_val"] = round(macd_now, 6)
                active_bots[user_id]["ind2_val"] = round(sig_now, 6)
                active_bots[user_id]["ind1_label"] = f"MACD({macd_fast},{macd_slow})"
                active_bots[user_id]["ind2_label"] = f"Sinyal({macd_signal})"
                active_bots[user_id]["ema_trend_val"] = round(ema_trend_val, 4) if ema_trend_val else None

                if not initialized:
                    initialized = True
                    active_bots[user_id]["status"] = "running"
                    trend_str = "YUKARI" if trend_up else "ASAGI"
                    print(f"Bot hazir. MACD={macd_now:.6f} Sinyal={sig_now:.6f} EMA{ema_trend}={ema_trend_val:.4f} Trend={trend_str}")
                    time.sleep(sleep_sec)
                    continue

                # MACD crossover tespiti
                macd_cross_up = macd_prev_val <= sig_prev_val and macd_now > sig_now
                macd_cross_down = macd_prev_val >= sig_prev_val and macd_now < sig_now
                ...
                if len(volumes) >= vol_period + 1:
                    ...
                    vol_ok = volumes[-1] > avg_vol
                    print(f"[{now}] Hacim: {volumes[-1]:.2f} Ort: {avg_vol:.2f} OK={vol_ok}")

                print(f"[{now}] MACD={macd_now:.6f} Sinyal={sig_now:.6f} EMA{ema_trend}={ema_trend_val:.4f} Trend={'↑' if trend_up else '↓'} Vol={vol_ok}")

                if macd_cross_up and trend_up and vol_ok:
                    signal = "BUY"
                    print(f"[{now}] BUY sinyali: MACD kesisti + trend yukari + hacim onayladi")
                elif macd_cross_down and trend_down and vol_ok:
                    signal = "SELL"
                    print(f"[{now}] SELL sinyali: MACD kesisti + trend asagi + hacim onayladi")
                elif macd_cross_up or macd_cross_down:
                    print(f"[{now}] MACD kesisti ama diger filtreler gecmedi (trend={trend_up if macd_cross_up else trend_down}, vol={vol_ok})")
...
def bot_status(current_user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "ind1_val": b.get("ind1_val"),
        "ind2_val": b.get("ind2_val"),
        "ind1_label": b.get("ind1_label"),
        "ind2_label": b.get("ind2_label"),
        "ema_trend_val": b.get("ema_trend_val"),
        ...
        "active_params": b.get("active_params"),
    }
