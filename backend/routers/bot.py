...
def sign(params):
    ...
    sig = hmac.new(get_secret_key().encode(), query.encode(), hashlib.sha256).hexdigest()
...
def open_position(symbol, side, usdt_amount, sl_pct, tp_pct):
    ...
    requests.post(
        f"{BASE_URL}/fapi/v1/order",
        headers=bheaders(),
        params={**sl_params, "signature": sign(sl_params)},
    )
    ...
    requests.post(
        f"{BASE_URL}/fapi/v1/order",
        headers=bheaders(),
        params={**tp_params, "signature": sign(tp_params)},
    )
