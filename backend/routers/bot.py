...
# Çalışan botları tutan sözlük: {user_id: {"thread": ..., "running": ..., "status": ...}}
active_bots = {}
...
def close_position(symbol, position_amt):
    ...
    print(f"[{datetime.now()}] Pozisyon kapatıldı: {side} {qty}")
...
def open_position(symbol, side, usdt_amount, sl_pct, tp_pct):
    ...
    print(f"[{datetime.now()}] Pozisyon açıldı {side} {qty} @ {price}")
...
def bot_loop(user_id, strategy):
    ...
    print(f"[{datetime.now()}] Bot başladı: user={user_id} {symbol} {timeframe} SMA{sma1_p}/{sma2_p}")
    ...
    while active_bots.get(user_id, {}).get("running", False):
        try:
            ...
            print(f"[{now}] SMA1={s1:.2f} SMA2={s2:.2f} → {signal}")
            ...
            print(f"[{datetime.now()}] Bot hatası: {e}")
            active_bots[user_id]["last_error"] = str(e)

        time.sleep(sleep_sec)

    # Bot durdu, pozisyonu kapat
    try:
        position = get_position(symbol)
        if position != 0:
            close_position(symbol, position)
...
def start_bot(
    ...
    if uid in active_bots and active_bots[uid].get("running"):
        raise HTTPException(400, "Bot zaten çalışıyor. Önce durdurun.")
    ...
    if not BINANCE_API_KEY or not BINANCE_SECRET_KEY:
        raise HTTPException(400, "Binance API key ayarlanmamış")
