...
def calc_bb(prices, period, std_mult):
    ...
    return round(upper, 6), round(mean, 6), round(lower, 6)
...
def bot_loop(user_id, strategy):
    ...
    # Başlangıçta mevcut sinyali kaydet, ilk DEĞİŞİMDE işlem yap
    prev_signal = None
    initialized = False
    ...
    while active_bots.get(user_id, {}).get("running", False):
        try:
            ...
            # Gösterge hesapla
            limit = max(sma1_p, sma2_p, bb_period, rsi_period) + 20
            ...
            if strategy_type == "sma":
                ...
                # Önceki mum değerleri (crossover tespiti için)
                ...
                # Mevcut durumu kaydet (ilk çalışmada)
                if not initialized:
                    prev_signal = "BUY" if s1 > s2 else "SELL"
                    initialized = True
                    active_bots[user_id]["status"] = "running"
                    print(f"Bot hazir. Mevcut durum: {prev_signal}. Kesisim bekleniyor...")
