...
def create_strategy(
    ...
    cur = db.execute(
        """INSERT INTO strategies
           (user_id,name,symbol,timeframe,days,sma1,sma2,rsi_period,rsi_ob,rsi_os,
            stop_loss,take_profit,notes,strategy_type,bb_period,bb_std,
            ema1,ema2,volume_filter,macd_fast,macd_slow,macd_signal,ema_trend,vol_period)
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (current_user["id"], body.name, body.symbol, body.timeframe, body.days,
         ...
         body.ema1, body.ema2, body.volume_filter,
         body.macd_fast, body.macd_slow, body.macd_signal,
         body.ema_trend, body.vol_period)
    )
    db.commit()
...
def update_strategy(
    ...
    db.execute(
        """UPDATE strategies SET
           ...
           strategy_type=?,bb_period=?,bb_std=?,ema1=?,ema2=?,volume_filter=?,
           macd_fast=?,macd_slow=?,macd_signal=?,ema_trend=?,vol_period=?,
