...
const NOTES = [
  ...
  {label:'Bollinger Bantları',html:`<strong>Bollinger Bant sinyalleri</strong><ul><li>Fiyat <strong>alt banda</strong> değer + RSI aşırı satım altında → AL</li><li>Fiyat <strong>üst banda</strong> değer + RSI aşırı alım üstünde → SAT</li><li>Standart ayar: 20 periyot, 2.0 standart sapma</li><li>Dar bant = düşük volatilite, kırılım yaklaşıyor olabilir</li></ul>`},
  {label:'EMA Crossover',html:`<strong>Golden Cross / Death Cross</strong><ul><li><strong>Golden Cross:</strong> EMA(50) > EMA(200) → AL — kısa vadeli momentum uzun vadeli trendi geçti</li><li><strong>Death Cross:</strong> EMA(50) < EMA(200) → SAT/SHORT</li><li><strong>Hacim filtresi:</strong> Kesişim anında hacim 20 mum ortalamasının üzerinde olmalı</li><li><strong>RSI onayı:</strong> Golden Cross'ta RSI 50+ olmalı</li><li><strong>EMA SL:</strong> Fiyat 200 EMA'nın altına kapanırsa pozisyon kapatılır</li></ul>`},
];
        ...
        rs.innerHTML = recent.map(s => `
          <div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:0.5px solid var(--border);">
            <div>
              ...
              <div style="font-size:11px;color:var(--text2);">${s.symbol} · ${TF_NAMES[s.timeframe]||s.timeframe} · ${StrategyPage.typeLabel(s.strategy_type)}</div>
            </div>
...
const StrategyPage = {
  ...
  typeLabel(t) {
    return {sma:'SMA Crossover', bb:'Bollinger Bantları', ema:'EMA Crossover'}[t] || t || 'SMA';
  },
  ...
  async render(el) {
    el.innerHTML = `
      ...
      <div style="display:grid;grid-template-columns:300px 1fr;gap:14px;align-items:start;" id="strat-layout">
        ...
        <div id="strat-detail"><div class="card" style="text-align:center;padding:40px;color:var(--text2);">Sol taraftan bir strateji seçin veya yeni oluşturun</div></div>
      </div>`;
  ...
  async loadStrategies() {
    ...
    if (!this.strategies.length) {
      el.innerHTML = '<div style="color:var(--text2);font-size:13px;padding:12px;">Henüz strateji yok.</div>';
      return;
    }
    el.innerHTML = this.strategies.map(s => `
      <div class="strat-card" data-id="${s.id}" onclick="StrategyPage.selectStrategy(${s.id})">
        ...
        <div class="strat-meta">
          ...
          <span class="pill">${this.typeLabel(s.strategy_type)}</span>
        </div>
  ...
  async selectStrategy(id) {
    ...
    document.getElementById('save-bt-btn').onclick = async () => {
      ...
      try {
        ...
        await Api.updateStrategy(id, {
          ...
          rsi_ob: gv('rsiOB'), rsi_os: gv('rsiOS'),
          stop_loss: gv('sl'), take_profit: gv('tp'), notes: s.notes,
          strategy_type: s.strategy_type||'sma',
          ...
          botStatusInterval = setInterval(async () => {
            ...
            if (posBar) {
              if (s2.running && s2.active_position) {
                ...
                document.getElementById('bp-info').textContent = `${ap.side} @ ${ap.entry_price} | SL: ${ap.sl_price} | TP: ${ap.tp_price} | Güncel: ${s2.current_price||'—'}`;
              } else {
    ...
    Api.botStatus().then(s2 => {
      ...
      if (s2.running) {
        btn.textContent = 'Botu Durdur';
        btn.className = 'btn-danger btn-sm';
        bar.textContent = `Çalışıyor: ${s2.symbol} ${s2.timeframe}`;
      }
    }).catch(()=>{});
  ...
  toggleStrategyFields() {
    ...
    const smaSection = document.getElementById('e-sma-section');
    const bbSection = document.getElementById('e-bb-section');
    const emaSection = document.getElementById('e-ema-section');
    if (smaSection) smaSection.style.display = type === 'sma' ? 'block' : 'none';
    if (bbSection) bbSection.style.display = type === 'bb' ? 'block' : 'none';
    if (emaSection) emaSection.style.display = type === 'ema' ? 'block' : 'none';
  },
  ...
  openEditor(id) {
        ...
        <div class="editor-grid">
          ...
          <div class="field" style="grid-column:1/-1">
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
              <input type="checkbox" id="e-volf" ${s&&s.volume_filter?'checked':'checked'}>
              Hacim filtresi (kesişim anında hacim 20 mum ortalamasının üzerinde olmalı)
            </label>
          </div>
        </div>
      </div>
    ...
    document.getElementById('e-save-btn').onclick = async () => {
      ...
      const data = {
        name, symbol: document.getElementById('e-sym').value||'BTCUSDT',
...
const ProfilePage = {
  ...
  async saveSettings() {
    try {
      await Api.updateSettings({ default_tf: document.getElementById('pref-tf').value, default_days: parseInt(document.getElementById('pref-days').value) });
